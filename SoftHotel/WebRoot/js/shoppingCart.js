...
$(".details").on("click","td.num button",function(){
  ...
  //alert(productId+"----"+type+"----"+uname+"------"+count);
  $.ajax({
    url:"room/order_updateCartDetail",
...
$(".details").on("click",".btn_delete",function(){
  ...
  var type=$(this).parent().siblings(".id").children("div").children().data('type');
  console.log(type);
  //alert(productId+"----"+type+"----"+uname);
  $.ajax({
    url:'room/order_deleteCardDtail',
...
$(".btn_sumbit").click(function() {
    var checked=$("td.id").children("input").prop("checked");
     if(checked){
       location.href="room/myorder_toOrderAdd";
     }
...
var order=function(){
   var uname=sessionStorage['loginName'];
  $.ajax({
    url:"room/myorder_queryHotelOrder",
    data:{uname:uname},
    success:function(data){
    	var str = $(data).html(); 
    	data = JSON.parse(str);
      ...
      if(data==null){
        ...
        $.each(data["orderlist"],function(i,orderlist){
          html+=`
            <tr class="title">
          ...
          $.each(data["productList"],function(i,productList){
          ...
          html+=`
                 ...
                 <td>${orderlist.status==0?"等待付款":(orderlist.status==1?"订单成功":"订单失败")}</td>
               </tr>
            `;
        });
      }
